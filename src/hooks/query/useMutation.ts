...
import { ExtendedMutationOptions } from '../../types/models';
...
>(
  ...
  return useMutationDefault<TData, TError, TVariables, any>(mutationFn, {
    ...
    onMutate: (data) => {
      ...
      if (queryKey) {
        previousData = queryClient.getQueryData<TUpdateData | undefined>(getQueryKey(data));

        queryClient.setQueryData<TUpdateData | undefined>(getQueryKey(data), (old) => {
          if (old) {
            return onUpdateData ? onUpdateData(old as any as TUpdateData, data) : old;
          }
